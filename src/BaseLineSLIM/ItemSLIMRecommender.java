...
public class ItemSLIMRecommender extends AbstractRecommender {
    /**
        ...
        for (int iter = 1; iter <= numIterations; iter++) {
            ...
            for (int itemIdx = 0; itemIdx < numItems; itemIdx++) {
                ...
                while (userItr.hasNext()) {
                    ...
                    // for each user, put her rating for item idx in this list
                    userRatingEntries[userRatingEntry.index()] = userRatingEntry.get();
                }

                // for each nearest neighbor nearestNeighborItemIdx, update coefficienMatrix(weights) by the coordinate
                // descent update rule
                for (Integer nearestNeighborItemIdx : nearestNeighborCollection) {
                    if (nearestNeighborItemIdx != itemIdx) {
                        ...
                        //The above line will retrieve all of the ratings, from all the users (that have rated this item nn)
                        // for a nn item
                        ...
                        while (nnUserRatingItr.hasNext()) {
                            // for each neighbor item, get the index of the user who rated it, and the rate
                        ...
                        //I did multiply it by 0.5 according to the formula
     ...
     */
    protected double predict(int userIdx, int itemIdx, int excludedItemIdx) { //excluded item = nn item
        //prediction of a nn user for the main item
        double predictRating = 0;
        Iterator<VectorEntry> itemEntryIterator = trainMatrix.colIterator(userIdx);//retrieve all the ratings of a user

        while (itemEntryIterator.hasNext()) {
            VectorEntry itemEntry = itemEntryIterator.next();
            int nearestNeighborItemIdx = itemEntry.index(); //index of the item is retrieved
            double nearestNeighborPredictRating = itemEntry.get(); //rating of the user on that item is retrieved

            if (itemNNs[itemIdx].contains(nearestNeighborItemIdx) && nearestNeighborItemIdx != excludedItemIdx) {
                //if this item is a nearest neighbor item and is not the item
